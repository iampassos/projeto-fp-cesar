...
class Registro:
    ...
    A classe `Registro` guarda os detalhes de uma sessão de corrida, seja um treino ou uma competição.
    Os atributos incluem o tipo de atividade, data, distância, duração, localização e clima.

    Atributos:
        tipo (str): Indica se é um "treino" ou "competição".
        data (Data): Objeto `Data` representando a data do evento, no formato DD/MM/AA.
    ...
    Métodos:
        __init__(self, tipo: str, data: str, distancia: int, duracao: int, localizacao: str, clima: str) -> None:
            Inicializa uma nova instância da classe `Registro` com os detalhes especificados.
        
        __str__(self) -> str:
            Retorna uma string formatada representando os detalhes do registro de corrida.
    """

    def __init__(self, tipo: str, data: str, distancia: int, duracao: int, localizacao: str, clima: str) -> None:
        """
